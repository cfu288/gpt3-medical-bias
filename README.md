...
The goal of this exploration is to see if there is any implicit bias shown by GPT when generating notes in medical context. We'll be evaluating this by exploring the word frequencies in the returned documents using NLP. We can compare the frequency of medications and medical conditions between different race cohorts. This is good to explore, as future healthcare applications that may choose to incorporate GPT models should be aware of these implicit biases.

As part of this repo, we provide our dataset generated by gpt-3.5 for free. To explore the gpt-3.5 response data (~50,000 documents) via sql queries, you can use our [interactive web app](https://cfu288.github.io/gpt3-medical-bias/). The raw text data can be [found here](/data/processed/documents/).

## Methods

### Generating the Medical History Documents From Cohort Data

We generate 2 cohorts of mock patients (see next section for information on how we do this) - one cohort for names more commonly associated with African-American individuals and one cohort of names more commonly associated with Caucasian individuals.

For each patient in our cohorts, we prompt OpenAI to generate a medical history note as if it was an emergency medicine physician seeing this undifferentiated patient.
...
We generate unique 10,000 documents for each chief complaint using our generated patient names, 5,000 from each of our race cohorts.

In the prompt template, we inject the name, age, and gender of the patient. Since age and gender are controlled between the two cohorts, the only independent variable between the cohorts should be the name of the patient. Note that we do not explicitly encode the race group of the name in the template or provide that information to OpenAI.
...
- It is unclear if the difference in medication and prescription patterns between the cohort reflects the underlying real-world prevelance of these conditions among these racial groups or just reflects bias in the training data.

## Limitations

- Results generated for gpt-3.5-turbo may not be generalizable to other LLMs.
- We use one specific prompt, it is unclear if variations on this prompt may reveal very different results.
- We only attempt to control for age and gender of the names associated with each race. We did not attempt to control for other data that may be encoded with each name (socioeconomic status).
